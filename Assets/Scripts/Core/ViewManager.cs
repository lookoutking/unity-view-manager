...
namespace Charizard.Views.Core
{
    public class ViewManager : MonoBehaviour
    {
        ...
        [SerializeField] private BaseView _startingView;

        [SerializeField] private BaseView[] _views;

        private BaseView _currentView;

        private readonly Stack<BaseView> _history = new Stack<BaseView>();
        ...
        public static void ShowWithProperties<T, TData>(TData data, bool remember = true) where T : BaseView<TData>
                                                                                    where TData : ViewProperties
        {
            for (int i = 0; i < s_instance._views.Length; i++)
            {
                if (s_instance._views[i] is T tView)
